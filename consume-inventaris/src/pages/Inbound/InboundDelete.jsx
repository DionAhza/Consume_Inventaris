...
export default function InboundDelete({ isOpen, onClose, endpoint, identitasColumn }) {
    if (!isOpen) {
        return null;
    }

    const [InboundDetail, setDataDetail] = useState({});
    const navigate = useNavigate();

    useEffect(() => {
        axios.get(endpoint['InboundDetail'], {
            headers: {
    ...
    function Inbound_Delete() {
        axios.delete(endpoint['inbound_delete'], {
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('access_token'),
            }
        })
                ...
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div className="p-4 md:p-5 text-center">
                        ...
                        <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete
                            ?</h3>
                        <button  onClick={()=>Inbound_Delete()} type="button" className="text-white bg-red-600 hover:bg-red-800
